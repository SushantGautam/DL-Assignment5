...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, prev_h, Wxh, Whh, b, tanh = cache

    dtanh = 1 - tanh**2                              
    dnext_t = dtanh * dnext_h                      

    dx = dnext_t.dot(Wxh.T)                        
    dprev_h = dnext_t.dot(Whh.T)                    
    dWx = (x.T).dot(dnext_t)                       
    dWh = (prev_h.T).dot(dnext_t)                  
    db = dnext_t.sum(axis=0)                      
    db = db.T.flatten()                            
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    grad_cache_forward = [None] * T
    h = np.zeros((N, T, H))

    for t in range(T):
        prev_h, prev_c, grad_cache_forward[t] = lstm_step_forward(x[:, t, :], prev_h, prev_c, Wx, Wh, b)
        h[:, t, :] = prev_h
    
    cache = grad_cache_forward, N, T, D, H
    pass
